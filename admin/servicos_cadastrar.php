...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($nome === "" || $preco <= 0 || $duracao <= 0 || $duracao_min <= 0) {
        header("Location: servicos_cadastrar.php?erro=1");
        exit;
    }
    ...
    if (!empty($_FILES["img"]["name"]) && is_uploaded_file($_FILES["img"]["tmp_name"])) {
        ...
        if (in_array($mime, $allowed, true)) {
            ...
            $destino  = $upload_dir . $img_name;
            move_uploaded_file($_FILES["img"]["tmp_name"], $destino);
        }
    }

    $sql = "
        INSERT INTO servicos (nome, preco, duracao, duracao_min, img, ativo)
        VALUES (:nome, :preco, :duracao, :duracao_min, :img, :ativo)
    ";
